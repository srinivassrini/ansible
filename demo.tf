...
resource "aws_instance" "appserver1" {
  ...
  ami                    = var.instance_ami
  instance_type          = var.instance_type
  security_groups        = [aws_security_group.app1.id,aws_security_group.web1.id,aws_security_group.bastion.id]
  key_name               = var.keyname
    user_data              = <<EOF
#!/bin/bash
# install httpd (Linux 2 version)
...
systemctl start httpd
systemctl enable httpd
echo "<h1>Hello World from Appserver1 $(hostname -f)</h1>" > /var/www/html/index.html
EOF
  tags = {
	Name = "App Server 1"
	}
...
resource "aws_instance" "appserver2" {
  ...
  ami                    = var.instance_ami
  instance_type          = var.instance_type
  security_groups        = [aws_security_group.app2.id,aws_security_group.web2.id]
  key_name               = var.keyname
    user_data              = <<EOF
#!/bin/bash
# install httpd (Linux 2 version)
...
systemctl start httpd
systemctl enable httpd
echo "<h1>Hello World from Appserver2 $(hostname -f)</h1>" > /var/www/html/index.html
EOF
  tags = {
	Name = "App Server 2"
	}
...
resource "aws_security_group" "dbsg" {
  ...
  ingress {
    ...
    to_port         = 80
    protocol        = "tcp"
    security_groups = [aws_security_group.app1.id, aws_security_group.app2.id] # allowing access from our example instance
  }
  ingress {
    from_port       = 3306
    to_port         = 3306
...
resource "aws_lb" "alb1" {
  ...
  security_groups    = [aws_security_group.web1.id,aws_security_group.web2.id,aws_security_group.bastion.id,aws_security_group.app1.id,aws_security_group.app2.id]
